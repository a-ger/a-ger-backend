...
import com.amazonaws.services.s3.transfer.Uploadimport
...
java.io.Serializableimport javax.persistence.*
/**
 * @Class : BoardUrl
...
@Entity
@DynamicUpdate
class BoardUrl : Serializable {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    var boardUrlId: Long? = null
    ...
    constructor(boardUrlId: Long?, board: Board?, url: String?) {
    ...
    override fun equals(o: Any?): Boolean {
        ...
        if (o !is BoardUrl) return false
        val other = o
        if (!other.canEqual(this as Any)) return false
        val `this$boardUrlId`: Any? = boardUrlId
        val `other$boardUrlId`: Any? = other.boardUrlId
        return if (if (`this$boardUrlId` == null) `other$boardUrlId` != null else `this$boardUrlId` != `other$boardUrlId`) false else true
    }

    protected fun canEqual(other: Any?): Boolean {
        return other is BoardUrl
    }

    override fun hashCode(): Int {
        val PRIME = 59
        var result = 1
        val `$boardUrlId`: Any? = boardUrlId
        result = result * PRIME + (`$boardUrlId`?.hashCode() ?: 43)
        return result
    }

    class BoardUrlBuilder internal constructor() {
        private var boardUrlId: Long? = null
        private var board: Board? = null
        private var url: String? = null
        fun boardUrlId(boardUrlId: Long?): BoardUrlBuilder {
            this.boardUrlId = boardUrlId
            return this
        }

        fun board(board: Board?): BoardUrlBuilder {
            this.board = board
            return this
        }

        fun url(url: String?): BoardUrlBuilder {
            this.url = url
            return this
        }

        fun build(): BoardUrl {
            return BoardUrl(boardUrlId, board, url)
        }

        override fun toString(): String {
            return "BoardUrl.BoardUrlBuilder(boardUrlId=" + boardUrlId + ", board=" + board + ", url=" + url + ")"
        }
    }

    companion object {
        fun builder(): BoardUrlBuilder {
            return BoardUrlBuilder()
        }
    }
}