...
class BoardRepositoryImpl : BoardRepositoryCustom {
    ...
    override fun findAllBoardPageableOrderByCreatedAtDesc(
        keyword: String,
        pageable: Pageable
    ): Slice<BoardSummaryResponse> {
        val boardJPAQuery = queryFactory
            .selectFrom(QBoard.board)
            .where(keywordListContains(keyword))
            .offset(pageable.offset)
            .limit((pageable.pageSize + 1).toLong())
        for (o in pageable.sort) {
            val pathBuilder: PathBuilder<*> = PathBuilder<Any?>(QBoard.board.type, QBoard.board.metadata)
            boardJPAQuery.orderBy(OrderSpecifier(if (o.isAscending) Order.ASC else Order.DESC, pathBuilder[o.property]))
        }
        val fetch = boardJPAQuery.fetch()
        val content: MutableList<BoardSummaryResponse> = ArrayList()
        for (board in fetch) {
            content.add(BoardSummaryResponse.Companion.toBoardSummaryResponse(board))
        }
        var hasNext = false
    ...
    override fun findBoardsByAccountId(accountId: Long?, pageable: Pageable): Slice<BoardSummaryResponse> {
        val boardQuery = queryFactory
            .selectFrom(QBoard.board)
            .where(QBoard.board.accountId.accountId.eq(accountId))
            .offset(pageable.offset)
            .limit((pageable.pageSize + 1).toLong()) //limit보다 한 개 더 들고온다.
        for (o in pageable.sort) {
            val pathBuilder: PathBuilder<*> = PathBuilder<Any?>(QBoard.board.type, QBoard.board.metadata)
            boardQuery.orderBy(OrderSpecifier(if (o.isAscending) Order.ASC else Order.DESC, pathBuilder[o.property]))
        }
        val fetch = boardQuery.fetch()
        val content: MutableList<BoardSummaryResponse> = ArrayList()
        for (board in fetch) {
            val countComment = queryFactory
                .selectFrom(QComment.comment)
                .where(QComment.comment.boardId.boardId.eq(board.boardId))
                .fetchCount()
            content.add(BoardSummaryResponse.Companion.toBoardSummaryResponse(board))
        }
        var hasNext = false
     ...
     * @Return : null
     */
    override fun addViewCntFromRedis(boardId: Long?, addCnt: Long?) {
        queryFactory
            .update(QBoard.board)
            .set(QBoard.board.boardViewCnt, addCnt)
     ...
     * @Return : Board
     */
    override fun addViewCnt(boardId: Long?): Board? {
        queryFactory
            .update(QBoard.board)
            .set(QBoard.board.boardViewCnt, QBoard.board.boardViewCnt.add(1))
            .where(QBoard.board.boardId.eq(boardId))
            .execute()
        return queryFactory
            .selectFrom(QBoard.board)
            .leftJoin(QBoard.board.urlList, QBoardUrl.boardUrl)
            .fetchJoin()
            .where(QBoard.board.boardId.eq(boardId), QBoard.board.eq(QBoardUrl.boardUrl.board))
            .fetchOne()
    }
    ...
    private fun keywordContains(keyword: String): BooleanExpression? {
        return if (ObjectUtils.isEmpty(keyword)) null else QBoard.board.title.contains(keyword)
    }
