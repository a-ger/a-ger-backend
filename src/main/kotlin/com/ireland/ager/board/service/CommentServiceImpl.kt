...
import com.ireland.ager.board.entity.*
...
class CommentServiceImpl {
     ...
     */
    fun saveComment(accessToken: String?, boardId: Long, commentRequest: CommentRequest): CommentResponse {
        ...
        val newComment =
            commentRepository!!.save<Comment>(CommentRequest.Companion.toComment(commentRequest, board, account))
        return CommentResponse.Companion.toCommentResponse(newComment)
    }
     ...
     * @Return : CommentResponse
     */
    fun updateComment(accessToken: String?, commentId: Long, commentRequest: CommentRequest): CommentResponse {
        val presentComment = commentRepository!!.findById(commentId).orElseThrow(
            Supplier<NotFoundException> { NotFoundException() })!!
        val account = accountService!!.findAccountByAccessToken(accessToken)
        if (account != presentComment.accountId) {
            throw UnAuthorizedAccessException()
        }
        val updateComment = commentRepository.save<Comment>(
            CommentRequest.Companion.toComment(
                commentRequest,
                presentComment.boardId,
                account
            )
        )
        return CommentResponse.Companion.toCommentResponse(updateComment)
    }
    ...
    fun deleteComment(accessToken: String?, commentId: Long) {
        val presentComment = commentRepository!!.findById(commentId).orElseThrow(
            Supplier<NotFoundException> { NotFoundException() })!!
        val account = accountService!!.findAccountByAccessToken(accessToken)
        if (account != presentComment.accountId) {
        ...
        commentRepository.deleteById(presentComment.commentId)
    }
}