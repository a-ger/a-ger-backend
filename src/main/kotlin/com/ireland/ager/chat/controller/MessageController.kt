...
com.ireland.ager.chat.entity.*import org.springframework.messaging.handler.annotation.Payloadimport
...
class MessageController {
    private val kafkaProductService: KafkaProductService? = null
    ...
    fun sendMessage(
        @ApiParam(value = "메시지 내용", required = true) @RequestBody message: MessageRequest
    ) {
    ...
    fun broadcastGroupMessage(
        @ApiParam(value = "채팅방 아이디", required = true) @DestinationVariable roomId: Long?,
        @ApiParam(value = "메시지", required = true) @Payload message: Message
    ): Message {
        return message
    }
}