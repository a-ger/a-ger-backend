...
class MessageRoomController {
     ...
     * @Return : ResponseEntity<SingleResult></SingleResult><MessageDetailsResponse>>
    </MessageDetailsResponse> */
    @GetMapping("/{roomId}")
    @ApiOperation(value = "채팅방 입장하기")
    fun roomEnter(
        @ApiParam(value = "채팅방 아이디", required = true) @PathVariable roomId: Long?,
        @ApiParam(value = "액세스 토큰", required = true) @RequestHeader("Authorization") accessToken: String
    ): ResponseEntity<SingleResult<MessageDetailsResponse?>?> {
        val splitToken = accessToken.split(" ").toTypedArray()
        val messageRoom = messageService!!.roomEnterByAccessToken(splitToken[1], roomId)
        return ResponseEntity(responseService!!.getSingleResult(messageRoom), HttpStatus.OK)
    }
     ...
     * @Return : ResponseEntity<SingleResult></SingleResult><RoomCreateResponse>>
    </RoomCreateResponse> */
    @PostMapping("/{productId}")
    @ApiOperation(value = "채팅방 생성")
    fun insertRoom(
        @ApiParam(value = "등록된 제품 아이디", required = true) @PathVariable productId: Long?,
        @ApiParam(value = "엑세스 토큰", required = true) @RequestHeader("Authorization") accessToken: String
    ): ResponseEntity<SingleResult<RoomCreateResponse?>?> {
        val splitToken = accessToken.split(" ").toTypedArray()
        val roomCreateResponse = messageService!!.insertRoom(productId, splitToken[1])
        return ResponseEntity(responseService!!.getSingleResult(roomCreateResponse), HttpStatus.CREATED)
    }
