package com.ireland.ager.chat.entityimportimport

com.fasterxml.jackson.annotation .JsonIgnoreimport com.ireland.ager.product.entity.*import java.io.Serializable
...
@Entity
@DynamicUpdate
class MessageRoom : BaseEntity, Serializable {
    @Id
    ...
    var sellerId: Account? = null

    @set:JsonIgnore
    @OneToOne
    @JsonIgnore
    @JoinColumn(name = "buyer_id")
    var buyerId: Account? = null

    @Enumerated(EnumType.STRING)
    var reviewStatus = ReviewStatus.NOTSALE

    @Enumerated(EnumType.STRING)
    var roomStatus = RoomStatus.FULL
    ...
    fun toCreateMessageRoom(product: Product, buyerId: Account?) {
        this.buyerId = buyerId
        sellerId = product.account
        this.product = product
    }

    fun updateRoomStatus(account: Account) {
        if (roomStatus == RoomStatus.EMPTY) return
        if (roomStatus == RoomStatus.FULL) {
            roomStatus = if (sellerId == account) RoomStatus.SELLEROUT else RoomStatus.BUYEROUT
        } else if (roomStatus == RoomStatus.SELLEROUT && buyerId == account) {
            roomStatus = RoomStatus.EMPTY
        } else if (roomStatus == RoomStatus.BUYEROUT && sellerId == account) { //buyerout
            roomStatus = RoomStatus.EMPTY
        }
    }

    class MessageRoomBuilder internal constructor() {
        private var roomId: Long? = null
        private var product: Product? = null
        private var sellerId: Account? = null
        private var buyerId: Account? = null
        private var reviewStatus: ReviewStatus? = null
        private var roomStatus: RoomStatus? = null
        fun roomId(roomId: Long?): MessageRoomBuilder {
            this.roomId = roomId
            return this
        }

        fun product(product: Product?): MessageRoomBuilder {
            this.product = product
            return this
        }

        fun sellerId(sellerId: Account?): MessageRoomBuilder {
            this.sellerId = sellerId
            return this
        }

        fun buyerId(buyerId: Account?): MessageRoomBuilder {
            this.buyerId = buyerId
            return this
        }

        fun reviewStatus(reviewStatus: ReviewStatus?): MessageRoomBuilder {
            this.reviewStatus = reviewStatus
            return this
        }

        fun roomStatus(roomStatus: RoomStatus?): MessageRoomBuilder {
            this.roomStatus = roomStatus
            return this
        }

        fun build(): MessageRoom {
            return MessageRoom(roomId, product, sellerId, buyerId, reviewStatus, roomStatus)
        }

        override fun toString(): String {
            return "MessageRoom.MessageRoomBuilder(roomId=" + roomId + ", product=" + product + ", sellerId=" + sellerId + ", buyerId=" + buyerId + ", reviewStatus=" + reviewStatus + ", roomStatus=" + roomStatus + ")"
        }
    }

    companion object {
        fun builder(): MessageRoomBuilder {
            return MessageRoomBuilder()
        }
    }
}