...
com.ireland.ager.chat.entity.*import org.springframework.data.domain.Pageableimport

org.springframework.data .domain.Sliceimport org.springframework.stereotype.Repository
@Repository
@RequiredArgsConstructor
class MessageRepositoryImpl : MessageRepositoryCustom {
    private val queryFactory: JPAQueryFactory? = null
    override fun findMessagesByMessageRequestOrderByCreatedAtDesc(
        messageRoomId: Long?,
        pageable: Pageable
    ): Slice<Message> {
        val messageList = queryFactory
            .selectFrom(QMessage.message1) //HINT 같은 룸 아이디만 가지고 온다. 여기서 많은 조회가 발생할거 같다.
            .where(QMessage.message1.messageRoom.roomId.eq(messageRoomId))
        ...
        var hasNext = false
        //마지막 페이지는 사이즈가 항상 작다.
        if (messageList.size > pageable.pageSize) {
            messageList.removeAt(pageable.pageSize)
            hasNext = true
        }
        return SliceImpl(messageList, pageable, hasNext)
    }
}