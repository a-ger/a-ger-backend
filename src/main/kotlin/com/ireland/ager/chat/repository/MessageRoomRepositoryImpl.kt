...
class MessageRoomRepositoryImpl : MessageRoomRepositoryCustom {
    private val queryFactory: JPAQueryFactory? = null
    override fun findMessageRoomsBySellerIdOrBuyerId(
        sellerId: Account,
        buyerId: Account?,
        pageable: Pageable
    ): Slice<MessageSummaryResponse> {
        val messageRoomList = queryFactory
            .selectFrom(QMessageRoom.messageRoom)
            .orderBy(QMessageRoom.messageRoom.updatedAt.desc())
            .offset(pageable.offset)
            .limit((pageable.pageSize + 1).toLong())
            .fetch()
        val content: MutableList<MessageSummaryResponse> = ArrayList()
        for (messageRoom in messageRoomList) {
            val message = queryFactory
                .selectFrom(QMessage.message1)
                .where(QMessage.message1.messageRoom.eq(messageRoom))
                .orderBy(QMessage.message1.createdAt.desc())
                .fetchFirst()
            if (sellerId == messageRoom.sellerId) {
                if (messageRoom.roomStatus == RoomStatus.BUYEROUT || messageRoom.roomStatus == RoomStatus.FULL) {
                    content.add(
                        MessageSummaryResponse.Companion.toMessageSummaryResponse(
                            messageRoom,
                            sellerId,
                            message
                        )
                ...
                if (messageRoom.roomStatus == RoomStatus.SELLEROUT || messageRoom.roomStatus == RoomStatus.FULL) {
                    content.add(
                        MessageSummaryResponse.Companion.toMessageSummaryResponse(
                            messageRoom,
                            buyerId,
