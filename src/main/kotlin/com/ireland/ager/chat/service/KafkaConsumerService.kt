...
class KafkaConsumerService {
    ...
    fun listen(message: MessageRequest) {
        KafkaConsumerService.log.info("sending via kafka listener..")
        //여기서 MessageRequest를 Message로 변환하고 정보 전달을 한다,,?
        val messageRoom = messageService!!.findByRoomId(message.roomId)
        messageRepository!!.save<Message>(MessageRequest.Companion.toMessage(message, messageRoom))
        template!!.convertAndSend("/topic/group/" + messageRoom!!.roomId, message)
    }
}