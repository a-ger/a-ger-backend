...
import com.ireland.ager.chat.entity.*
 ...
 */
class MessageService {
    private val messageRoomRepository: MessageRoomRepository? = null
     ...
     * @Return : MessageRoom
     */
    fun findByRoomId(roomId: Long): MessageRoom {
     ...
     * @Return :
     */
    fun deleteById(accessToken: String?, roomId: Long) {
        val accountByAccessToken = accountService!!.findAccountByAccessToken(accessToken)
        val messageRoom = findByRoomId(roomId)
        val messageRoomAfterDelete = deleteAccountWithMessageRoom(messageRoom, accountByAccessToken)
        if (messageRoomAfterDelete.roomStatus == RoomStatus.EMPTY) {
     ...
     * @Return : MessageRoom
     */
    fun deleteAccountWithMessageRoom(messageRoom: MessageRoom, account: Account?): MessageRoom {
        if (!(account == messageRoom.sellerId || account == messageRoom.buyerId)) {
     ...
     */
    fun insertRoom(productId: Long?, accessToken: String?): RoomCreateResponse {
        val buyerAccount = accountService!!.findAccountByAccessToken(accessToken)
        val sellProduct = productService!!.findProductById(productId)
        if (buyerAccount == sellProduct.account) {
            throw UnAuthorizedChatException()
        }
        val messageRoom = messageRoomRepository!!.findMessageRoomByProductAndBuyerId(sellProduct, buyerAccount)
        if (messageRoom!!.isPresent) {
            return RoomCreateResponse.Companion.toRoomCreateResponse(messageRoom.get())
        }
        val insertMessageRoom = MessageRoom()
        insertMessageRoom.toCreateMessageRoom(sellProduct, buyerAccount)
        val savedRoom = messageRoomRepository.save(insertMessageRoom)
        //웰컴 메시지
        val message: Message = MessageRequest.Companion.toMessage(
            MessageRequest(
                savedRoom.roomId, buyerAccount.accountId, "상품을 구매하고 싶어요"
            ), savedRoom
        )
        messageRepository!!.save(message)
        return RoomCreateResponse.Companion.toRoomCreateResponse(insertMessageRoom)
    }
     ...
     */
    fun roomEnterByAccessToken(accessToken: String?, roomId: Long?): MessageDetailsResponse {
        val messageRoombyRoomId = messageRoomRepository!!.findMessageRoomByRoomId(roomId).orElseThrow(
            Supplier<NotFoundException> { NotFoundException() })
        val accountByAccessToken = accountService!!.findAccountByAccessToken(accessToken)
        if (!(accountByAccessToken == messageRoombyRoomId!!.sellerId || accountByAccessToken == messageRoombyRoomId.buyerId)) {
            throw UnAuthorizedAccessException()
        }
        return MessageDetailsResponse.Companion.toMessageDetailsResponse(messageRoombyRoomId)
    }
    ...
    fun findRoomByAccessToken(accessToken: String?, pageable: Pageable?): Slice<MessageSummaryResponse?>? {
