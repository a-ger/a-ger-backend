...
class ProductServiceImpl {
    ...
    fun findProductAllByCreatedAtDesc(
        category: Category?,
        keyword: String?,
        pageable: Pageable?
    ...
    fun createProduct(
        ...
        val thumbNailUrl = uploadService!!.makeThumbNail(multipartFile[0])
        val uploadImagesUrl = uploadService.uploadImages(multipartFile)
        val product = productRequest!!.toProduct(account, uploadImagesUrl, thumbNailUrl)
        productRepository!!.save(product)
        return ProductResponse.Companion.toProductResponse(product, account)
    }
    ...
    fun addViewCntToRedis(productId: Long) {
        val key = "productViewCnt::$productId"
        val valueOperations = redisTemplate!!.opsForValue()
        if (valueOperations[key] == null) valueOperations.set(
            key, productRepository!!.findProductViewCnt(productId).toString(),
            Duration.ofMinutes(5)
        ) else valueOperations.increment(key)
        ProductServiceImpl.log.info("value:{}", valueOperations[key])
    }
    ...
    fun deleteViewCntCacheFromRedis() {
        val redisKeys: Set<String> = redisTemplate!!.keys("productViewCnt*")
        val it = redisKeys.iterator()
        while (it.hasNext()) {
            val data = it.next()
            val productId = data.split("::").toTypedArray()[1].toLong()
    ...
    fun updateProductById(
        ...
        val productById = findProductById(productId)
        val accountByAccessToken = accountService!!.findAccountByAccessToken(accessToken)
        if (productById.account != accountByAccessToken) {
            throw UnAuthorizedTokenException()
        }
        validateFileExists(multipartFile)
        val currentFileImageUrlList = productById.urlList
        val currentFileThumbnailUrl = productById.thumbNailUrl
        uploadService!!.delete(currentFileImageUrlList, currentFileThumbnailUrl)
        productById.deleteUrl()
        val firstImage = multipartFile[0]
        var updateFileImageUrlList: List<String?>? = ArrayList()
        try {
            updateFileImageUrlList = uploadService.uploadImages(multipartFile)
            productById.thumbNailUrl = uploadService.makeThumbNail(firstImage)
        } catch (e: IllegalStateException) {
            e.printStackTrace()
        } catch (e: IOException) {
            e.printStackTrace()
        }
        val toProductUpdate = productUpdateRequest!!.toProductUpdate(productById, updateFileImageUrlList)
        productRepository!!.save(toProductUpdate)
        return ProductResponse.Companion.toProductResponse(toProductUpdate, accountByAccessToken)
    }
     ...
     * @Return : null
     */
    fun deleteProductById(productId: Long, accessToken: String?) {
        val productById = productRepository!!.findById(productId).orElseThrow(
            Supplier<NotFoundException> { NotFoundException() })
        if (productById.account.accountId != accountService!!.findAccountByAccessToken(accessToken).accountId) {
            throw UnAuthorizedTokenException()
        }
        uploadService!!.delete(productById.urlList, productById.thumbNailUrl)
    ...
    fun validateUploadForm(bindingResult: BindingResult) {
        if (bindingResult.errorCount >= 3) throw InvaildFormException()
        if (bindingResult.hasErrors()) {
            bindingResult.allErrors.forEach(Consumer { objectError: ObjectError ->
                ...
                when (Objects.requireNonNull(errorCode)) {
                    "3010" -> throw InvaildProductTitleException()
                    "3020", "3021" -> throw InvaildProductPriceException()
                    "3030" -> throw InvaildProductDetailException()
                    "3040" -> throw InvaildProductCategoryException()
                    "3050" -> throw InvaildProductStatusException()
                }
            })
        }
    }
