...
class ReviewRepositoryImpl : ReviewRepositoryCustom {
    ...
    override fun findReviewsByAccountId(accountId: Long?, pageable: Pageable): Slice<ReviewResponse> {
        val reviewQuery = queryFactory
            .selectFrom(QReview.review)
            .where(QReview.review.sellerId.accountId.eq(accountId))
            .offset(pageable.offset)
            .limit((pageable.pageSize + 1).toLong())
        for (o in pageable.sort) {
            val pathBuilder: PathBuilder<*> = PathBuilder<Any?>(QReview.review.type, QReview.review.metadata)
            reviewQuery.orderBy(OrderSpecifier(if (o.isAscending) Order.ASC else Order.DESC, pathBuilder[o.property]))
        }
        val content: List<ReviewResponse> =
            ArrayList<ReviewResponse>(ReviewResponse.Companion.toReviewResponse(reviewQuery.fetch()))
        var hasNext = false
