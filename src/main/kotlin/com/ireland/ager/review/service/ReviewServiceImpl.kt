...
class ReviewServiceImpl {
     ...
     */
    fun postReview(roomId: Long, reviewRequest: ReviewRequest, accessToken: String?): ReviewResponse {
        val messageRoom = messageRoomRepository!!.findById(roomId).orElseThrow(
            Supplier<NotFoundException> { NotFoundException() })!!
        val accountByAccessToken = accountService!!.findAccountByAccessToken(accessToken)
        if (accountByAccessToken.accountId != messageRoom.buyerId.accountId) {
            throw UnAuthorizedAccessException()
        }
        return if (messageRoom.reviewStatus == ReviewStatus.SALE) {
            val review: Review = ReviewRequest.Companion.toReview(
                reviewRequest,
                messageRoom.sellerId,
                messageRoom.product,
                accountByAccessToken
            )
            messageRoom.reviewStatus = ReviewStatus.POST
            messageRoomRepository.save(messageRoom)
            reviewRepository!!.save(review)
            ReviewResponse.Companion.toReviewResponse(review)
        } else throw DuplicateReviewException()
    }
}