...
com.ireland.ager.product.entity.*import javax.persistence.*
...
class Trade {
    ...
    constructor(tradeId: Long?, product: Product?, buyerId: Account?) {
    ...
    override fun equals(o: Any?): Boolean {
        ...
        if (o !is Trade) return false
        val other = o
        if (!other.canEqual(this as Any)) return false
        val `this$tradeId`: Any? = tradeId
        val `other$tradeId`: Any? = other.tradeId
        return if (if (`this$tradeId` == null) `other$tradeId` != null else `this$tradeId` != `other$tradeId`) false else true
    }
    ...
    override fun hashCode(): Int {
        val PRIME = 59
        var result = 1
        val `$tradeId`: Any? = tradeId
        result = result * PRIME + (`$tradeId`?.hashCode() ?: 43)
        return result
    }

    class TradeBuilder internal constructor() {
        private var tradeId: Long? = null
        private var product: Product? = null
        private var buyerId: Account? = null
        fun tradeId(tradeId: Long?): TradeBuilder {
            this.tradeId = tradeId
            return this
        }

        fun product(product: Product?): TradeBuilder {
            this.product = product
            return this
        }

        fun buyerId(buyerId: Account?): TradeBuilder {
            this.buyerId = buyerId
            return this
        }

        fun build(): Trade {
            return Trade(tradeId, product, buyerId)
        }

        override fun toString(): String {
            return "Trade.TradeBuilder(tradeId=" + tradeId + ", product=" + product + ", buyerId=" + buyerId + ")"
        }
    }
    ...
    companion object {
         ...
         * @Return : Trade
         */
        fun toTrade(product: Product?, buyerId: Account?): Trade {
            return builder()
                .product(product)
                .buyerId(buyerId)
                .build()
