...
class TradeRepositoryImpl : TradeRepositoryCustom {
    ...
    override fun findBuyProductsByAccountId(accountId: Long?, pageable: Pageable): Slice<ProductThumbResponse> {
        val tradeQuery = queryFactory
            .selectFrom(QTrade.trade)
            .where(QTrade.trade.buyerId.accountId.eq(accountId))
            .offset(pageable.offset)
            .limit((pageable.pageSize + 1).toLong())
        for (o in pageable.sort) {
            val pathBuilder: PathBuilder<*> = PathBuilder<Any?>(QTrade.trade.type, QTrade.trade.metadata)
            tradeQuery.orderBy(OrderSpecifier(if (o.isAscending) Order.ASC else Order.DESC, pathBuilder[o.property]))
        }
        val tradeList = tradeQuery.fetch()
        val productList: MutableList<Product?> = ArrayList()
        for (trade in tradeList) productList.add(trade.product)
        val content: List<ProductThumbResponse> =
            ArrayList<ProductThumbResponse>(ProductThumbResponse.Companion.toProductListResponse(productList))
        var hasNext = false
