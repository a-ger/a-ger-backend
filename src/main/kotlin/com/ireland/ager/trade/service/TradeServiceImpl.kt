...
class TradeServiceImpl {
    ...
    fun isUpdated(
        ...
        val product = messageRoom.product
        val accountByAccessToken = accountService!!.findAccountByAccessToken(accessToken)
        if (product.account != accountByAccessToken) {
            throw UnAuthorizedAccessException()
        }
        product.status = ProductStatus.valueOf(updateStatus!!)
        if (product.status == ProductStatus.COMPLETE) {
            messageRoom.reviewStatus = ReviewStatus.SALE
            messageRoomRepository.save(messageRoom)
        }
        tradeRepository!!.save<Trade>(
            Trade.Companion.toTrade(
                productRepository!!.save(product), messageRoom.buyerId
            )
        )
    }
}